...
SORT_VOD = ['new', 'old'][int(__addon__.getSetting('sort_vod'))]
PROFILE = int(__addon__.getSetting('select_profile'))
...
def dp_main():
    ...
    if wavve_id and wavve_pw:
        # check login
        if not Wavve().GetCredential(wavve_id, wavve_pw, PROFILE):
            # login failed
    ...
    if 'celllist' in items:
        for list in items['celllist']:
            if 'contenttype=vod' in url:
                ...
                title = cleantext(list['title_list'][0]['text'])
                ...
                for bodylist in list['event_list'][0]['bodylist']:
                    if bodylist.startswith('uicode:'):
                        ...
                        pass
                item['sub_title'] = cleantext(list['title_list'][1]['text']).replace(' $O$ ',' ')
                infoLabels = {'mediatype': 'episode', "title": title,
                              "plot": item['sub_title']
                              if 'sub_title' in item else item['title']}
                addDir(item, infoLabels)
            else:
                item = {'category': submode, 'title': cleantext(list['title_list'][0]['text']), 'pageno': pageno,
...
def dp_vod_list(p):
    ...
    code = p['pcode']
    items = Wavve().GetVODEpisode(code, pageno_ep, SORT_VOD)
    addon_log('%s!!!!' % items)
    if 'list' in items:
        for item in items['list']:
            title = cleantext(item['episodetitle']).encode('UTF-8')
            epno = item['episodenumber'].encode('UTF-8')
            if title:
                if epno not in title: title += ' %s회' % epno
            else: title = '%s회' % epno
            plot = cleantext(item['synopsis'].encode('utf8') if 'synopsis' in item
                     else title)
            ...
            if plot: param['plot'] = quote(plot.encode('UTF-8'))
            param['programtitle'] = quote(cleantext(item['programtitle']).encode('UTF-8'))
            infoLabels = {'mediatype': 'episode', "title": cleantext(item['episodetitle']),
                          "plot": plot}
...
def dp_movie_sub2(p):
    ...
    item = dict(category=submode,
                url='apis.pooq.co.kr/cf/movie/contents?WeekDay=all&broadcastid=132257&came=movie&contenttype=movie&genre=all&limit=20&offset=0&price=all&uiparent=FN0&uirank=0&uitype=MN4&apikey=E5F3E0D30947AA5440556471321BB6D9',
                type=type,
                title='전체',
                order=order)
    addDir(item)
...
def dp_list(p):
    ...
    if 'celllist' in items:
        for list in items['celllist']:
            ...
            infoLabels = {'mediatype': 'episode', "title": title}
            if mode == 'Live_Sub':
                item['sub_title'] = cleantext(list['title_list'][1]['text'])
    ...
    if quality_idx is not None:
        ...
        if p['category'] == 'VOD_Sub_List':
            ...
            if programitems['programid']:
                ...
                param['ptitle'] = quote(cleantext(programitems['programtitle']).encode('UTF-8'))
                param['pimg'] = programitems['programimage']
            else:
                param['pcode'] = p['pcode']
                param['ptitle'] = cleantext(p['ptitle'])
                param['pimg'] = p['pimg']
            if programitems['programsynopsis']:
                param['pplot'] = quote(cleantext(programitems['programsynopsis']).encode('UTF-8'))
            elif 'pplot' in p:
                param['pplot'] = p['pplot']
            title = cleantext(programitems['episodetitle']).encode('UTF-8')
            programtitle = cleantext(programitems['programtitle']).encode('UTF-8')
            ...
            plot = cleantext(programitems['synopsis']).encode('utf8')
            ...
            plot = cleantext(movieitems['synopsis']).encode('utf8')
        else: plot = ''
        if 'img' in p:
