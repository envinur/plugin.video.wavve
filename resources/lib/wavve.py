...
class Wavve(object):
    ...
    def GetWeekday(self):
        ...
        return day[weekday]
    def GetCredential(self, user_id, user_pw, PROFILE):
        isLogin = False
        try:
            url_path = 'login'
            params = self.param
            params.update(drm='wm')
            payload = {'id': user_id,
                       'password': user_pw,
                       "profile": '0',
                       'pushid': '',
            ...
            if credential:
                profilecount = int(resp_json['profilecount'])
                if PROFILE < profilecount and profilecount > 1:
                    params.update(credential=credential)
    ...
    def GetVODEpisode(self, pcode, pageno_ep, order):
        params = self.param
        params.update({'orderby': order,
                       'limit': self.limit,
